...
export default function Home() {
  ...
  // based on the ChrisNFT's held by the user for which they haven't claimed the tokens
  const [tokensToBeClaimed, setTokensToBeClaimed] = useState(zero);
  // balanceOfChrisTokens keeps track of number of chris tokens owned by an address
  const [balanceOfChrisTokens, setBalanceOfChrisTokens] =
    useState(zero);
  ...
  /**
   * getBalanceOfChrisTokens: checks the balance of Chris Tokens's held by an address
   */
  const getBalanceOfChrisTokens = async () => {
    try {
      // Get the provider from web3Modal, which in our case is MetaMask
      // No need for the Signer here, as we are only reading state from the blockchain
      const provider = await getProviderOrSigner();
      ...
      setBalanceOfChrisTokens(balance);
    } catch (err) {
      console.error(err);
      setBalanceOfChrisTokens(zero);
    }
  };
   ...
   */
  const mintChrisToken = async (amount) => {
    try {
      ...
      await tx.wait();
      setLoading(false);
      window.alert("Sucessfully minted Chris Tokens");
      await getBalanceOfChrisTokens();
  ...
  /**
   * claimChrisTokens: Helps the user Chris Tokens
   */
  const claimChrisTokens = async () => {
    try {
      // We need a Signer here since this is a 'write' transaction.
      // Create an instance of tokenContract
      const signer = await getProviderOrSigner(true);
      ...
      await tx.wait();
      setLoading(false);
      window.alert("Successfully claimed Chris Tokens");
      await getBalanceOfChrisTokens();
      await getTotalTokensMinted();
      await getTokensToBeClaimed();
    } catch (err) {
  ...
  */
    ...
    if (tokensToBeClaimed > 0) {
      return (
        <div>
          ...
          <button className={styles.button} onClick={claimChrisTokens}>
            Claim Tokens
          </button>
        </div>
      );
    ...
    return (
      <div style={{ display: "flex-col" }}>
        ...
        <button
          ...
          onClick={() => mintChrisToken(tokenAmount)}
        >
          Mint Tokens
        </button>
      </div>
  ...
  return (
    <div>
      <Head>
        <title>Chris Tokens</title>
        <meta name="description" content="ICO-Dapp" />
        <link rel="icon" href="/favicon.ico" />
      </Head>
      <div className={styles.main}>
        <div>
          <h1 className={styles.title}>Welcome to Chris ICO!</h1>
          <div className={styles.description}>
            You can claim or mint Chris tokens here
          </div>
          {walletConnected ? (
            <div>
              <div className={styles.description}>
                {/* Format Ether helps us in converting a BigNumber to string */}
                You have minted {utils.formatEther(balanceOfChrisTokens)}{" "}
                Chris Tokens
              </div>
              <div className={styles.description}>
                {/* Format Ether helps us in converting a BigNumber to string */}
